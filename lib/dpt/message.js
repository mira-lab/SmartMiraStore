...
const ping = {
  encode: function (obj) {
    return [int2buffer(obj.version), endpoint.encode(obj.from), endpoint.encode(obj.to), timestamp.encode(obj.timestamp)];
  },
  decode: function (payload) {
    return {
...
const neighbours = {
  encode: function (obj) {
    return [obj.peers.map(peer => endpoint.encode(peer).concat(peer.id)), timestamp.encode(obj.timestamp)];
  },
  decode: function (payload) {
    return {
